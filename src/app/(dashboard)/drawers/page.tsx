...
import {
  Folder, FolderOpen, ChevronRight, ChevronLeft, File,
  ShieldCheck, Users, Briefcase, Search,
} from "lucide-react";
...
export default function DrawersPage() {
  ...
  const [systemFolders, setSystemFolders] = useState<DrawerFolder[]>([]);
  const [clientFolders, setClientFolders] = useState<DrawerFolder[]>([]);
  ...
  // System folders load once — never re-fetch on page/search change
  useEffect(() => {
    const loadSystem = async () => {
      setLoadingSystem(true);
      try {
        const iaMasterData = await IAMasterService.getLatest().catch(() => null);
        const sys: DrawerFolder[] = [];
  ...
  // Client folders re-fetch on page or search change (server-side both)
  const loadClients = useCallback(async (pageNum: number, search: string) => {
    setLoadingClients(true);
    try {
      const { clients, total } = await MasterDataService.listClients({
        page: pageNum,
        limit: PAGE_SIZE,
  ...
  useEffect(() => {
    loadClients(page, debouncedSearch);
  ...
  // Open folder — fetch documents lazily on first open
  const openFolder = async (folder: DrawerFolder) => {
    if (folder.type === "CLIENT" && folder.documents.length === 0) {
      try {
        const detail = await MasterDataService.getClient(folder.id);
        const docs = detail.documents || [];
        const updated = { ...folder, documents: docs };
        setClientFolders((prev) => prev.map((f) => (f.id === folder.id ? updated : f)));
  ...
  const reloadActiveFolder = async () => {
    ...
    try {
      const detail = await MasterDataService.getClient(activeFolder.id);
      const docs = detail.documents || [];
      setActiveFolder({ ...activeFolder, documents: docs });
      setClientFolders((prev) => prev.map((f) => (f.id === activeFolder.id ? { ...f, documents: docs } : f)));
    } catch {}
  };
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 space-y-6 animate-in fade-in duration-500">
      ...
      {!activeFolder ? (
        <div className="space-y-6">
          {systemFolders.length === 0 && clientFolders.length === 0 && !loadingClients ? (
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {/* System folders always render first — no loading state needed */}
                {systemFolders.map((folder) => (
                  <FolderCard key={folder.id} folder={folder} onClick={() => openFolder(folder)} />
                ))}

                {/* Client folders or loading skeletons */}
                {loadingClients
                  ? Array.from({ length: PAGE_SIZE }).map((_, i) => (
                      <Skeleton key={`sk-${i}`} className="h-44 w-full rounded-xl" />
                    ))
                  : clientFolders.length === 0 && debouncedSearch
                  ? null
              ...
              {!loadingClients && debouncedSearch && clientFolders.length === 0 && (
                <div className="text-center py-12 bg-primary/5 rounded-xl border border-primary/10 border-dashed">
                  <p className="text-muted-foreground">No clients match &quot;{debouncedSearch}&quot;.</p>
                </div>
              )}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-primary/10">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className="border-primary/20 gap-1"
                    >
...
// ── Extracted card to avoid re-creating inline functions per render ──
function FolderCard({ folder, onClick }: { folder: DrawerFolder; onClick: () => void }) {
  const isSystem = folder.type !== "CLIENT";
  const Icon = folder.icon;
  return (
    <Card
      className={`group cursor-pointer transition-all hover:shadow-lg bg-card/80 backdrop-blur-sm relative overflow-hidden ${
        isSystem
      ...
      <CardContent className="p-6 flex flex-col items-center text-center">
        <div
          className={`w-16 h-16 rounded-lg flex items-center justify-center mb-4 transition-colors ${
            isSystem ? "bg-amber-500/10 group-hover:bg-amber-500/20" : "bg-primary/5 group-hover:bg-primary/10"
          }`}
        >
          <Icon
            className={`w-8 h-8 transition-colors ${
              isSystem
                ? "text-amber-600 group-hover:text-amber-500 fill-amber-500/10"
                : "text-primary/70 group-hover:text-primary fill-primary/10"
            }`}
          />
        </div>
        ...
        <div
          className={`mt-4 pt-4 border-t w-full flex justify-between items-center text-xs text-muted-foreground ${
            isSystem ? "border-amber-500/10" : "border-primary/10"
          }`}
        >
          <span className="flex items-center gap-1">
            <File className="w-3 h-3" />
            {folder.documents?.length > 0 ? `${folder.documents.length} Files` : "Open to view"}
          </span>
          <span
            className={`${isSystem ? "text-amber-600" : "text-primary"} opacity-0 group-hover:opacity-100 transition-opacity font-semibold`}
          >
            Open
          </span>
        </div>
