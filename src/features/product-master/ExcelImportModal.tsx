...
import { ProductType, ExcelPreviewRow, ProductMasterService } from "@/core/services/product-master.service";
...
export function ExcelImportModal({ open, onClose, productType, onImported }: Props) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setLoading(true);
    try {
      const preview = await ProductMasterService.excelPreview(productType, file);
  ...
  async function handleImport() {
    ...
    setError("");
    try {
      const res = await ProductMasterService.excelImport(productType, rows);
      setResult(res);
      setStep("done");
      onImported();
    } catch {
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="w-[95vw] max-w-5xl flex flex-col max-h-[90vh] overflow-hidden">
        <DialogHeader className="shrink-0">
          <DialogTitle className="capitalize">Excel Import — {productType.replace(/-/g, " ")}</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="rounded-lg border border-dashed border-border p-8 text-center">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Download the demo template first to ensure correct column format.
              </p>
            </div>
              ...
              <table className="text-sm border-collapse" style={{ minWidth: `${columns.length * 180 + 100}px` }}>
                ...
                <tbody>
                  {rows.map((row, rowIndex) => (
                    <tr
                      key={rowIndex}
                      className={`border-t border-border ${row.status === "existing" ? "bg-muted/40" : ""}`}
                    >
