          ...
          {questions.length === 0 ? (
            ...
            questions.map((q, idx) => {
              ...
              return (
                ...
                >
                  <CardHeader className={`transition-all duration-300 ${isActive ? 'p-6 pb-4' : 'p-5 pb-3'}`}>
                   <div className="flex items-center gap-4">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm transition-all shrink-0 ${isActive ? 'bg-primary text-black' : 'bg-muted/30 text-muted-foreground/50'}`}>
                         {idx + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                         {isActive ? (
                            <Input
                              ...
                              className="bg-transparent border-none text-xl font-bold focus-visible:ring-0 shadow-none px-0 h-auto py-0 placeholder:text-muted-foreground/30"
                              autoFocus
                            />
                         ) : (
                            <h3 className="text-base font-semibold text-foreground/80 truncate max-w-lg">{q.text || "Untitled Question"}</h3>
                         )}
                      </div>
                   </div>
                </CardHeader>

                <CardContent className={`px-6 pt-2 pb-6 space-y-4 transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 h-0 p-0 overflow-hidden'}`}>
                   <div className="flex items-center justify-between">
                      <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Answer Options</Label>
                      <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); addOption(idx); }} className="h-7 gap-1.5 text-primary/60 text-xs font-medium hover:bg-primary/5 px-2 rounded-md transition-all">
                        ...
                        <div key={opt.id} className="flex gap-2 items-center">
                           <div className="flex-1">
                             <Input
                               ...
                               className="h-9 bg-primary/[0.02] border-primary/10 text-sm px-3 rounded-lg focus-visible:ring-primary/10 placeholder:text-muted-foreground/30"
                             />
                           </div>
                           <div className="w-20 relative">
                             <Input
                               ...
                               className="h-9 text-sm text-center border-primary/10 bg-primary/5 rounded-lg focus-visible:ring-primary/10"
                             />
                             <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] font-bold opacity-30 text-primary uppercase pointer-events-none">pts</span>
                           </div>
                           <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); removeOption(idx, oIdx); }} className="h-9 w-9 text-destructive/30 hover:text-destructive hover:bg-destructive/5 rounded-lg shrink-0 transition-all">
                ...
                {isActive && (
                   <CardFooter className="px-6 py-3 bg-muted/[0.03] border-t border-primary/5 flex justify-end items-center gap-2">
                      ...
                      <Separator orientation="vertical" className="h-5 bg-primary/10 mx-1" />
                      <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); removeQuestion(idx); }} className="h-8 px-3 gap-1.5 text-sm font-medium text-destructive/50 hover:text-destructive hover:bg-destructive/5 rounded-md">
